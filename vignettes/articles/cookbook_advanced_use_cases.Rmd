...
The bank can use company-level PACTA results to compare counterparties' production plans against company-specific benchmarks, based on the same transition scenarios used for sector-level targets. By conducting the analysis in 2020 and assessing deviations over a five-year horizon, the bank can identify which companies are proactively transitioning away from their coal power business and which ones are not, informing its engagement or exposure management strategy. The following code is a an example of how to obtain the appropriate subset of company level results and restructure the data to easily calculate a range of metrics to identify and analyse company-level misalignment:


```{r use_case_three_risk_mangement_coal_power_tech_mix, warning = FALSE}
power_company_tech_mix <- market_share_targets_company %>% 
  filter(
    region == "global",
    sector == "power",
  ...
  mutate(technology_share = round(technology_share, 3) * 100) %>% 
  pivot_wider(
    names_from = c("metric", "year"),
    values_from = "technology_share",
    names_prefix = "tech_share_"
  ) %>% 
  select(-"tech_share_target_sds_2020")

```

```{r use_case_three_risk_mangement_coal_power_tech_mix_table, echo = FALSE}
power_company_tech_mix %>%
  result_table(
    title = "Company-level technology share of coal power capacity",
    subtitle = "restructured for input into company metrics"
  ) %>%
  gt::cols_width(
    name_abcd ~ px(110),
    sector ~ px(80),
  ...
  gt::cols_label(
    ...
    tech_share_projected_2020 = "Projected tech share 2020 (%)",
    tech_share_projected_2025 = "Projected tech share 2025 (%)",
    tech_share_target_sds_2025 = "Target tech share 2025 (%)"
  )

```

```{r use_case_three_risk_mangement_coal_power_prod, warning = FALSE}
power_company_production <- market_share_targets_company %>% 
  filter(
    region == "global",
    sector == "power",
    metric %in% c("projected", "target_sds"),
  ...
  select(name_abcd, sector, technology, metric, year, production) %>%
  mutate(production = round(production)) %>% 
  pivot_wider(
    names_from = c("metric", "year"),
    values_from = "production",
    names_prefix = "prod_"
  ) %>% 
  select(-"prod_target_sds_2020")
  
```

```{r use_case_three_risk_mangement_result_power, echo = FALSE}
power_company_production %>%
  result_table(
    title = "Company-level capacity level of coal power",
    subtitle = "restructured for input into company metrics"
  ) %>%
  gt::cols_width(
    name_abcd ~ px(110),
  ...
  gt::cols_label(
    ...
    sector = "Sector",
    technology = "Technology",
    prod_projected_2020 = "Projected capacity 2020 (MW)",
    prod_projected_2025 = "Projected capacity 2025 (MW)",
    prod_target_sds_2025 = "Target capacity 2025 (MW)"
  )
```

The bank can now determine the most misaligned companies in the coal power business (based on technology share or production capacities) and engage with these companies to understand the reasons for the deviation. It can then help them develop a transition plan to reduce their exposure to high-carbon activities. A few calculations that help with determining the right approach to engaging these companies are laid out below:

- let $T_{c, i}(t)$ be the technology share of company $c$ and technology $i$ at time $t$, and
- let $p_{c, i}(t)$ be the production (or capacity) level of company $c$ and technology $i$ at time $t$.

(A) The bank can calculate the percent deviation of the projected technology share $T^{projected}$ relative to the target technology share $T^{target}$ of a company $c$ for a technology $i$ in $t_5$ as follows:

$$T_{c, i}^{deviation}(t_{5}) = \frac{T_{c, i}^{projected}(t_{5}) - T_{c, i}^{target}(t_{5})}{T_{c, i}^{target}(t_{5})}$$
This can be a good first indicator of misalignment at the company level. A related calculation would be to calculate the percent deviation of the projected production (or capacity) level $p^{projected}$ relative to the target production level $p^{target}$ of a company $c$ for a technology $i$ in $t_5$, again as follows:

$$p_{c, i}^{deviation}(t_{5}) = \frac{p_{c, i}^{projected}(t_{5}) - p_{c, i}^{target}(t_{5})}{p_{c, i}^{target}(t_{5})}$$
...
(B) The bank can calculate the relative deviation of the difference between projected production $p_{c, i}^{projected}(t_{5})$ and target production $p_{c, i}^{target}(t_{5})$ relative to the initial production level $p_{c, i}^{projected}(t_{0})$ of a company $c$ for a technology $i$ as follows:

$$p_{c, i}^{deviation}(t_{0,5}) = \frac{p_{c, i}^{projected}(t_{5}) - p_{c, i}^{target}(t_{5})}{p_{c, i}^{projected}(t_{0})}$$

Comparing the difference in $t_5$ with the initial company production level highlights how large the remaining effort can be considered. For example, if a company misses its coal power target in $t_5$ by 5MW, this would be relatively more severe if the company initially operated 10MW of coal power capacity and relatively less severe if it initially operated 50MW of coal capacity. After understanding the severity of the misalignment, it may be necessary to determine the underlying drivers of the misalignment.

(C) The bank can now calculate the relative change in production (or capacity) level $p_{c, i}^{projected}$ over time from $t_0$ to $t_5$ of a company $c$ for a technology $i$ as follows:

$$p_{c, i}^{change}(t_{0,5}) = \frac{p_{c, i}^{projected}(t_{5}) - p_{c, i}^{projected}(t_{0})}{p_{c, i}^{projected}(t_{0})}$$
...
```{r use_case_three_risk_mangement_cement_ei, warning = FALSE}
cement_company_ei <- sda_targets_company %>% 
  filter(
    region == "global",
    sector == "cement",
  ...
  mutate(emission_factor_value = round(emission_factor_value, 3)) %>% 
  pivot_wider(
    names_from = c("emission_factor_metric", "year"),
    values_from = "emission_factor_value",
    names_prefix = "ei_"
  ) %>% 
  select(-"ei_target_demo_2020")

```

Similarly, the bank can now determine the most misaligned cement companies (based on projected emission intensity) and engage with these companies as well. Following up on this, the bank can again help these companies develop a transition plan to reduce their emission intensity and align their business more with energy transition targets or trends.

```{r use_case_three_risk_mangement_cement_ei_table, echo = FALSE}
cement_company_ei %>%
  result_table(
    title = "Company-level emission intensity per tonne of cement",
    subtitle = "restructured for input into company metrics"
  ) %>%
  gt::cols_width(
    name_abcd ~ px(110),
  ...
  gt::cols_label(
    name_abcd = "Company",
    sector = "Sector",
    ei_projected_2020 = "Projected EI 2020 (t CO2eq/t cement)",
    ei_projected_2025 = "Projected EI 2025 (t CO2eq/t cement)",
    ei_target_demo_2025 = "Target EI 2025 (t CO2eq/t cement)"
  ) %>% gt::tab_options(
    ihtml.use_pagination = TRUE
  )
```

The production related-equations from above can be adjusted for emission intensity sectors. The bank can use these to identify the most misaligned companies and understand the reasons for their deviation.

- let $I_{c, s}(t)$ be the emission intensity in emissions per physical unit of production or capacity of a company $c$ in sector $s$ at time $t$

(A) The bank can calculate the percent deviation of the projected emission intensity $I_{c, s}^{projected}(t_{5})$ relative to the target emission intensity $I_{c, s}^{target}(t_{5})$ of a company $c$ in a sector $s$ in $t_5$ as follows:

$$I_{c, s}^{deviation}(t_{5}) = \frac{I_{c, s}^{projected}(t_{5}) - I_{c, s}^{target}(t_{5})}{I_{c, s}^{target}(t_{5})}$$

This first indicator of misalignment can also reveal that all misaligned cement companies seem to be relatively far off their target in this example, which may lead the bank to inspect if there are reasons beyond the immediate control of the companies for this misalignment. In the cement sector, one might assume that there is not yet a technology to significantly lower the emission intensity available to any company, which would limit their ability to meet the emissions target in the short term.

The remaining calculations presented above would translate to emission intensity sectors as follows:

(B) Relative deviation of the difference between projected emission intensity $I_{c, s}^{projected}(t_{5})$ and target emission intensity $I_{c, s}^{target}(t_{5})$ relative to the initial emission intensity $I_{c, s}^{projected}(t_{0})$ of a company $c$ in a sector $s$:

$$I_{c, s}^{deviation}(t_{0,5}) = \frac{I_{c, s}^{projected}(t_{5}) - I_{c, s}^{target}(t_{5})}{I_{c, s}^{projected}(t_{0})}$$

(C) Relative change in emission intensity $I_{c, s}^{projected}$ of a company $c$ over time from $t_0$ to $t_5$ in a sector $s$:

$$I_{c, s}^{change}(t_{0,5}) = \frac{I_{c, s}^{projected}(t_{5}) - I_{c, s}^{projected}(t_{0})}{I_{c, s}^{projected}(t_{0})}$$
