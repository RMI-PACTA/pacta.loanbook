...
cat(paste0("- `", names(r2dii.data::abcd_demo), "`", collapse = "\n"))
```
...
cat(paste0("- `", names(r2dii.data::scenario_demo_2020), "`", collapse = "\n"))
```
...
cat(paste0("- `", names(r2dii.data::co2_intensity_scenario_demo), "`", collapse = "\n"))
```
...
cat(paste0("- `", names(r2dii.data::loanbook_demo), "`", collapse = "\n"))
```
...
In case the user cannot obtain sector classification codes of any of the classification systems featured in `pacta.loanbook::sector_classifications` (currently the following classification systems are featured: `r unique(pacta.loanbook::sector_classifications$code_system)`), the user can provide a manually created sector classification file for matching the loan book to the ABCD instead. Generally, any such manually prepared sector classification file must follow the format of `pacta.loanbook::sector_classifications`. It is recommended to use the built in sector classifications if possible, as mapping your own sector classification to the PACTA sectors can be complex and time consuming.
...
R is the programming language that the `{pacta.loanboo}` package is written in. You can download R from the [Comprehensive R Archive Network (CRAN)](https://cran.r-project.org/).
...
You can install the development versions of the `{pacta.loanbook}` R package from GitHub with:
...
The `{pacta.loanbook}` R package depends on a number of other R packages. These dependencies will be installed automatically when you install the `{pacta.loanbook}` R packages The required packages are:
...
imports <- imports[!unlist(lapply(imports, grepl, pattern = "^r2dii[.]"))]
