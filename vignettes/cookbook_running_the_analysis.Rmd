...
Assuming you have prepared and/or procured all required input data sets as described in the section "Preparatory Steps" **LINK**, implementing the PACTA for Banks analysis now follows the following main steps:
...
-   Manually validate matches between the loanbook and abcd datasets, including slecting the appropriate match in cases where more than one company in the abcd receives a score higher than the threshold value
...
### Optional: Exclude misclassified loans from the analysis
...
There are a large variety of possible visualizations stemming from the outputs of `r2dii.analysis::target_market_share()` and `r2dii.analysis::target_sda()`. Below, we highlight a couple of common plots that can easily be created using the `{r2dii.plot}` package.
...
**ADD ISNTRUCTIONS** Write to file
