...
pacta_loanbook_attach <- function() {
...
pacta_loanbook_attach_message <- function(to_load) {
  if (length(to_load) == 0) {
    return(NULL)
  }

  header <- cli::rule(
    left = cli::style_bold("Attaching core pacta.loanbook packages"),
    right = paste0("pacta.loanbook ", package_version_h("pacta.loanbook"))
  )
